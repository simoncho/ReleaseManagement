...
CREATE Procedure [dbo].[Dev_Get_DeployHistory]
...
SELECT BD.[DefinitionId]
  ...
  --LastBuild
  OUTER APPLY (SELECT B.StartTime, B.FinishTime, B.BuildNumber, BQ.Quality, B.PartitionId, ISNULL(S1.code_name, CONVERT(VARCHAR(255), B.BuildStatus)) AS code_name, AO.DisplayName
  ...
  --LEFT OUTER JOIN [$(TFS_Server)].[$(TFS_Database)].[dbo].[tbl_Build] B2  WITH(NOLOCK) on BD.LastGoodBuildUri = B2.BuildUri AND B2.Deleted =0 AND B1.PartitionId=BD.PartitionId
  OUTER APPLY (SELECT * 
 ...
 WHERE BD.GroupId in (2, 3) AND BD.PartitionId=1
   ...
   AND B1.StartTime > DATEADD(hh, @hh_Diff_Timezone, @StartDate)
